  ...
  init(params) {
    ...
    if (params.value === undefined) {
      ...
      var flag = `<img border="0" width="15" height="10" alt="${
        params.value
      } flag"  src="https://flags.fmcdn.net/data/flags/mini/${
...
const booleanCellRenderer = (props) => {
  ...
  if (valueCleaned === false) {
    return (
      <span title="false" className="ag-icon ag-icon-cross content-icon" />
    );
  }
...
export default function Example() {
  ...
  const getContextMenuItems = (params) => {
    ...
    result.push({
      ...
      //shortcut: 'Alt + M',
      action: () => {
        const message = `You clicked a custom menu item on cell ${
          params.value ? params.value : '<empty>'
        }`;
            ...
            pie: {
              ...
              tooltip: {
                renderer: (params) => ({
                  content:
                    '$' +
                    formatThousands(Math.round(params.datum[params.angleKey])),
                }),
              },
            },
          },
        ...
        cartesian: {
          ...
          series: {
            column: {
              tooltip: {
                renderer: (params) => ({
                  content:
                    '$' +
                    formatThousands(Math.round(params.datum[params.yKey])),
                }),
              },
            },
            bar: {
            ...
            scatter: {
              tooltip: {
                renderer: (params) => {
                  const label = params.labelKey
                    ? params.datum[params.labelKey] + '<br>'
                    : '';
                  const xValue =
                    params.xName +
                    ': $' +
                    formatThousands(params.datum[params.xKey]);
                  const yValue =
                    params.yName +
                    ': $' +
